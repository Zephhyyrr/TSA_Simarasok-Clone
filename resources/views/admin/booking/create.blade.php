...
@section('content')
    ...
    <div class="col-6">
        ...
        <form action="/admin/booking" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select name="homestay_id" class="form-control @error('homestay_id') is-invalid @enderror" id="">
                    @foreach ($homestay as $item)
                        <option value="{{ $item->id }}" @if (old('homestay_id') == $item->id) selected @endif>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
                @error('homestay_id')
            ...
            <div class="mb-3">
                ...
                <div class="form-group">
                    <input type="date" class="form-control @error('checkin') is-invalid @enderror" name="checkin" value="{{ old('checkin') }}">
                    <input type="date" class="form-control @error('checkout') is-invalid @enderror" name="checkout" value="{{ old('checkout') }}">
                </div>
